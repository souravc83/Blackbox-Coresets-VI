...
class BayesBatchNorm2d(nn.BatchNorm2d):
    def forward(self, x):
        if x.shape == 4:
            return super().forward(x)
        d0, d1 = x.shape[:2]
...
def conv3x3(in_planes, out_planes, stride=1, mc_samples=10, init_sd=0.1):
    return  VIConv2d(in_channels=in_planes, out_channels=out_planes, kernel_size=3, stride=stride, padding=1, bias=False, mc_samples=mc_samples, init_sd=init_sd)
...
class BasicBlock(nn.Module):
    ...
    def __init__(self, in_planes, planes, stride=1, mc_samples=10, init_sd=0.1):
        super(BasicBlock, self).__init__()
        self.conv1 = conv3x3(in_planes, planes, stride, mc_samples, init_sd)
        self.bn1 = BayesBatchNorm2d(planes)
        self.conv2 = conv3x3(in_planes=planes, out_planes=planes, 
                             mc_samples=mc_samples, init_sd=init_sd)
        self.bn2 = BayesBatchNorm2d(planes)

        self.shortcut = nn.Sequential()
        if stride != 1 or in_planes != self.expansion * planes:
            self.shortcut = nn.Sequential(
                 VIConv2d(in_channels=in_planes, out_channels= self.expansion * planes, kernel_size=1, stride=stride, bias=False, mc_samples=mc_samples, init_sd=init_sd),
...
class Bottleneck(nn.Module):
    ...
    def __init__(self, in_planes, planes, stride=1, mc_samples=10, init_sd=0.1):
        ...
        self.conv1 = VIConv2d(in_channels=in_planes, out_channels=planes, kernel_size=1, bias=False, mc_samples=mc_samples, init_sd=init_sd)
        self.bn1 = BayesBatchNorm2d(planes)
        self.conv2 = VIConv2d(in_channels=planes, out_channels=planes, kernel_size=3, stride=stride, padding=1, bias=False, mc_samples=mc_samples, init_sd=init_sd)
        self.bn2 = BayesBatchNorm2d(planes)
        self.conv3 = VIConv2d(in_channels=planes, out_channels=self.expansion * planes, kernel_size=1, bias=False,  mc_samples=mc_samples, init_sd=init_sd)
        self.bn3 = nn.BatchNorm2d(self.expansion * planes)

        self.shortcut = nn.Sequential()
        if stride != 1 or in_planes != self.expansion * planes:
            self.shortcut = nn.Sequential(
                VIConv2d(in_planes, self.expansion * planes, kernel_size=1, stride=stride, 
                         bias=False, mc_samples=mc_samples, init_sd=init_sd),
                nn.BatchNorm2d(self.expansion * planes)
            )
...
class ResNet_32x32(nn.Module):
    def __init__(self, block, num_blocks, channel=3, num_classes=10, mc_samples=10, init_sd=0.1, record_embedding: bool = False,
                 no_grad: bool = False):
        super().__init__()
        self.in_planes = 64
        self.mc_samples = mc_samples
        ...
        self.conv1 = conv3x3(in_planes=channel, out_planes=64, 
                             mc_samples=mc_samples, init_sd=init_sd)
        ...
        self.layer4 = self._make_layer(block, 512, num_blocks[3], stride=2,
                                       mc_samples=mc_samples, init_sd=init_sd)
        self.linear = VILinear(in_features=512 * block.expansion, out_features= num_classes, mc_samples=mc_samples, init_sd=init_sd)

        self.no_grad = no_grad

    def _make_layer(self, block, planes, num_blocks, stride, mc_samples, init_sd):
...
def ResNet(arch: str, channel: int, num_classes: int, mc_samples, init_sd):
    arch = arch.lower()
    if arch == "resnet18":
        net = ResNet_32x32(BasicBlock, [2, 2, 2, 2], channel=channel, num_classes=num_classes,
                           mc_samples=mc_samples, init_sd=init_sd, record_embedding=False, no_grad=False)
    elif arch == "resnet34":
        net = ResNet_32x32(BasicBlock, [3, 4, 6, 3], channel=channel, num_classes=num_classes,
                           mc_samples=mc_samples, init_sd=init_sd, 
                           record_embedding=False, no_grad=False)
    elif arch == "resnet50":
...
def make_resnet(mc_samples, init_sd):
    return ResNet(arch="resnet18", channel=3, num_classes=10, mc_samples=mc_samples, init_sd=init_sd)
